...
function BuiltinSimulation({
  ...
  return (
    <div className="w-full max-w-full overflow-hidden rounded-sm border border-vlab-rule-strong bg-vlab-surface-alt">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-vlab-rule-strong bg-white px-3 py-2.5 sm:px-4">
        <div className="flex min-w-0 items-center gap-2.5">
          <span className="h-2 w-2 shrink-0 rounded-full bg-green-600" />
          <span className="truncate text-sm font-semibold text-vlab-800">{title}</span>
        </div>
        <span className="shrink-0 font-mono text-[10px] uppercase tracking-[0.08em] text-vlab-muted">
...
function NativeSimulation({
  ...
  return (
    <div
      ...
      className="w-full max-w-full overflow-hidden rounded-sm border border-vlab-rule-strong bg-vlab-surface-alt"
    >
...
function SimNotice({ heading, body }: { heading: string; body: string }) {
  return (
    <div className="w-full max-w-full rounded-sm border border-vlab-rule-strong bg-vlab-surface-alt px-4 py-8 text-center sm:py-10">
      <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-sm border border-vlab-rule-strong bg-white">
        <AlertTriangle className="h-5 w-5 text-vlab-muted" />
      </div>
      <p className="text-sm font-semibold text-vlab-800">{heading}</p>
      <p className="mx-auto mt-1.5 max-w-md text-[13px] leading-relaxed text-vlab-muted">{body}</p>
    </div>
  )
}
...
function TinkercadSimulation({
  ...
  if (!designId) {
    return (
      <div className="border border-vlab-rule-strong bg-vlab-surface-alt flex flex-col items-center justify-center gap-3 py-16 text-center px-6">
        <div className="w-12 h-12 rounded-lg bg-vlab-surface flex items-center justify-center">
          <MonitorPlay className="w-6 h-6 text-vlab-300" />
        </div>
        <p className="text-sm text-vlab-muted">No simulation linked to this section yet.</p>
      </div>
    )
  }
  ...
  return (
    <div className="border border-vlab-rule-strong overflow-hidden bg-white"
      >
      ...
      {simActive ? (
        ...
        <div>
          ...
          <div className="flex items-center justify-between gap-3 px-5 py-3 border-b border-vlab-rule bg-white">
            <div className="flex items-center gap-2.5">
              <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
              <span className="text-sm font-semibold text-vlab-ink">{title}</span>
            </div>
            <div className="flex items-center gap-2">
              <a href={tinkercadUrl} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-xs text-vlab-muted hover:text-vlab-ink transition-colors px-3 py-1.5 border border-vlab-rule-strong hover:border-vlab-rule-strong">
                <ExternalLink className="w-3.5 h-3.5" />
                Full screen
              </a>
              <button onClick={() => setSimActive(false)}
                className="text-xs text-vlab-muted hover:text-vlab-ink transition-colors px-3 py-1.5 border border-vlab-rule-strong hover:border-vlab-rule-strong">
                ← Back
              </button>
            </div>
          </div>
          ...
          {/* Info bar */}
          <div className="flex items-center gap-2 px-5 py-2.5 bg-vlab-surface-alt border-b border-vlab-rule text-xs text-vlab-muted">
            <span>Click <strong className="text-vlab-ink">Start Simulation</strong> in the viewer below to run the circuit.</span>
          </div>
        ...
        <div>
          {/* Preview image */}
          <div className="relative bg-vlab-surface-alt flex items-center justify-center"
            style={{ minHeight: 280 }}>
            {previewLoading ? (
              <div className="flex flex-col items-center gap-3 py-16">
                <Loader2 className="w-8 h-8 text-vlab-300 animate-spin" />
                <p className="text-xs text-vlab-muted">Loading preview…</p>
              </div>
            ) : previewUrl ? (
              <img
                src={previewUrl}
                ...
                style={{ maxHeight: 320 }}
                onError={(e) => { (e.target as HTMLImageElement).parentElement!.innerHTML = '<div class="py-16 flex flex-col items-center gap-2"><svg xmlns=\'http://www.w3.org/2000/svg\' width=\'40\' height=\'40\' viewBox=\'0 0 24 24\' fill=\'none\' stroke=\'#c7d3de\' stroke-width=\'1.5\'><rect x=\'2\' y=\'3\' width=\'20\' height=\'14\' rx=\'2\'/><path d=\'M8 21h8M12 17v4\'/></svg><p class=\'text-xs text-vlab-300\'>Preview unavailable</p></div>' }}
              />
            ) : (
              <div className="flex flex-col items-center gap-3 py-16">
                <MonitorPlay className="w-10 h-10 text-vlab-300" />
                <p className="text-xs text-vlab-muted">No preview available</p>
              </div>
            )}
            ...
            {!previewLoading && (
              ...
              >
                <div className="w-16 h-16 rounded-full bg-vlab-600 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
          ...
          {/* Footer */}
          <div className="border-t border-vlab-rule px-5 py-4 space-y-3">
            {/* Title + launch */}
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-vlab-ink">{title}</p>
                <p className="text-xs text-vlab-muted mt-0.5">Tinkercad Circuit Simulation</p>
              </div>
              <button
                onClick={() => setSimActive(true)}
                className="inline-flex items-center gap-2 px-4 py-2 border border-vlab-600 bg-vlab-600 text-white text-sm font-semibold hover:bg-vlab-700 transition-colors shrink-0"
              >
            ...
            <div className="flex items-start gap-2.5 pt-1 border-t border-vlab-rule">
              <LogIn className="w-3.5 h-3.5 text-vlab-300 shrink-0 mt-0.5" />
              <p className="text-xs text-vlab-muted leading-relaxed">
                If the simulation doesn&apos;t load,{' '}
                <a href={loginUrl} target="_blank" rel="noopener noreferrer"
                  className="text-vlab-ink font-medium underline underline-offset-2 hover:text-vlab-600 transition-colors">
                  sign in to Tinkercad
                </a>{' '}
                first, then click{' '}
                <button onClick={() => setSimActive(true)} className="text-vlab-ink font-medium underline underline-offset-2 hover:text-vlab-600 transition-colors">
                  Launch
                </button>
                {' '}again.
              </p>
