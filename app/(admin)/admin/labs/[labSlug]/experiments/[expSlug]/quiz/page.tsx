...
async function getQuizData(labSlug: string, expSlug: string) {
  ...
  return { lab, experiment, quiz, questions: questions ?? [] }
}
...
export default async function QuizEditorPage({
  ...
  const data = await getQuizData(labSlug, expSlug)
  if (!data) notFound()

  const { lab, experiment, quiz, questions } = data
  ...
  return (
    <div className="p-8">
      <div className="flex items-center gap-2 mb-6 text-sm text-[#6a6a6a]">
        ...
        <span>/</span>
        <span className="text-[#222222]">Quiz</span>
      </div>

      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-[#222222]">{quiz.title}</h1>
          <p className="text-sm text-[#6a6a6a] mt-1 capitalize">{quiz.type} quiz</p>
        </div>
