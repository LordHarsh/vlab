  ...
  return (
    <div className="flex items-center gap-2">
      <button type="button" onClick={() => onChange(!value)}
        className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${value ? 'bg-vlab-600' : 'bg-vlab-rule-strong'}`}>
      ...
      <span className="text-xs text-vlab-muted">{label}</span>
    </div>
  )
}
  ...
  return (
    <div className="flex items-center gap-2">
      <button type="submit" disabled={isPending}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-vlab-600 text-white text-xs font-medium hover:bg-vlab-700 transition-colors disabled:opacity-50">
        {isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}
        Save
      </button>
      <button type="button" onClick={onCancel}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-vlab-surface text-vlab-muted text-xs font-medium hover:bg-vlab-rule transition-colors">
        <X className="w-3 h-3" />
        Cancel
      </button>
    </div>
...
function SimulationEditor({ section, onClose }: { section: Section; onClose: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-3 border border-vlab-rule rounded-lg p-4 bg-vlab-surface-alt space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-medium text-vlab-muted mb-1">Section Title</label>
        ...
        <div>
          <label className="block text-xs font-medium text-vlab-muted mb-1">Height (px)</label>
      ...
      <div>
        <label className="block text-xs font-medium text-vlab-muted mb-1">
          Tinkercad Design ID <span className="text-vlab-600">*</span>
        </label>
        <input value={designId} onChange={(e) => setDesignId(e.target.value)} placeholder="e.g. XXXXXXXXXXXXXXXX"
          className="w-full px-3 py-2 rounded-lg border border-vlab-rule-strong text-sm font-mono text-vlab-ink focus:outline-none focus:border-vlab-600 focus:ring-1 focus:ring-vlab-600 transition bg-white" />
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-3 border border-vlab-rule rounded-lg p-4 bg-vlab-surface-alt space-y-3">
      ...
      {section.type !== 'quiz' && section.type !== 'feedback' && (
        <div>
          <label className="block text-xs font-medium text-vlab-muted mb-1">
            Content <span className="text-vlab-300 font-normal">(JSON)</span>
          </label>
          <textarea value={raw} onChange={(e) => setRaw(e.target.value)} rows={10} spellCheck={false}
            className="w-full px-3 py-2 rounded-lg border border-vlab-rule-strong text-xs font-mono text-vlab-ink focus:outline-none focus:border-vlab-600 focus:ring-1 focus:ring-vlab-600 transition bg-white resize-y" />
          {HINTS[section.type] && (
            <p className="text-xs text-vlab-muted mt-1 font-mono truncate">Shape: {HINTS[section.type]}</p>
          )}
        </div>
      )}
      {(section.type === 'quiz' || section.type === 'feedback') && (
        <p className="text-xs text-vlab-muted bg-vlab-surface rounded-lg px-3 py-2">
...
export function SectionsList({
  ...
  if (sections.length === 0) {
    return <p className="text-sm text-vlab-muted text-center py-6">No sections yet. Add one below.</p>
  }
  ...
  return (
    <div className="space-y-2">
      {sections.map((section, idx) => {
        ...
        return (
          <div key={section.id} className="rounded-lg border border-vlab-rule hover:border-vlab-rule-strong transition-colors">
            <div className="flex items-center gap-3 p-3">
              {/* Order controls */}
              <div className="flex flex-col gap-0.5 shrink-0">
                <button onClick={() => handleMove(section, 'up')} disabled={isMovePending || idx === 0}
                  className="p-1 rounded-lg hover:bg-vlab-surface disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
                  <ArrowUp className="w-3 h-3 text-vlab-muted" />
                </button>
                <button onClick={() => handleMove(section, 'down')} disabled={isMovePending || idx === sections.length - 1}
                  className="p-1 rounded-lg hover:bg-vlab-surface disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
                  <ArrowDown className="w-3 h-3 text-vlab-muted" />
                </button>
              </div>

              {/* Type badge */}
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium capitalize shrink-0 ${TYPE_COLORS[section.type] ?? 'bg-vlab-surface text-vlab-muted'}`}>
                {section.type}
                {section.type === 'quiz' && section.quiz_type && ` · ${QUIZ_TYPE_LABEL[section.quiz_type] ?? section.quiz_type}`}
              </span>

              {/* Title */}
              <span className="flex-1 text-sm text-vlab-ink truncate">
                {section.title ?? <span className="text-vlab-300 italic">Untitled</span>}
              </span>

              {/* Required badge */}
              {section.is_required && (
                <span className="text-xs text-vlab-600 font-medium shrink-0">Required</span>
              )}
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                    isEditing
                      ...
                      : 'text-vlab-muted bg-vlab-surface hover:bg-vlab-rule'
                  }`}
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium text-vlab-muted bg-vlab-surface hover:bg-vlab-rule transition-colors disabled:opacity-50"
                >
